...
public class Mezcla{
	...
	public static void mezclaEquilibradaNombre(String nombreArchivo){    //recibira la direccion del archivo en cadena
		...
		do{
			...
			while(tamanoListaOriginal != arr.size()){//(arr1.size() != 0) | (arr2.size() != 0) ){	//mientras una lista u otra no esté vacía
		...
		bw.close();             //se cierra bufferedWriter
	...
	public static void mezclaEquilibradaApellido(String nombreArchivo){    //recibira la direccion del archivo en cadena
		...
		FileManager leer = new FileManager(1);  //creacion del objeto para manipulacion de archivos (para el ordenamiento)
		File archivo = new File(nombreArchivo); 
		...
		int inicioAux = 0;
		int finAux = 0;
		int indice1 = 0;	//marcador para las comas
		int indice2 = 0;	//marcador para las comas
		...
		do{
			...
			while( arr.size() != 0){	//mientras la lita original no este vacia.
				...
				if(arr.size() > 1){	//si hay mas de un elemento
					indice1 = arr.get(l-1).indexOf(",") + 2;
					indice2 = arr.get(l).indexOf(",") + 2;
					while( (arr.size() > l) && ((arr.get(l-1).substring(indice1,arr.get(l-1).length()).compareTo(arr.get(l).substring(indice2,arr.get(l).length()))) <= 0)){	//compara al elemento...
																						//...actual con el anterior...
																					//...siempre y cuando nos encontremos...
																				//...en indices validos.
						fin+=1;	// fin al final dirá el numero de elementos que entreran en la lista aux 1 o 2.
			...
			while( (arr1.size() != 0) | (arr2.size() != 0) ){	//mientras una lista u otra no esté vacía
				...
				if(arr1.size() > 0){
					indice1 = arr1.get(k-1).indexOf(",") + 2;
				}
				if(arr1.size() > 1){
					indice2 = arr1.get(k).indexOf(",") + 2;
				}

				while( (arr1.size() > k) && (arr1.get(k-1).substring(indice1,arr1.get(k-1).length()).compareTo(arr1.get(k).substring(indice2,arr1.get(k).length())) <= 0 ) ){
					k++;	//contará el numero de elementos que estan ordenados en orden natural...
							//...en la lista aux 1.
				...
				if(arr2.size() > 0){
					indice1 = arr2.get(m-1).indexOf(",") + 2;
				}
				if(arr2.size() > 1){
					indice2 = arr2.get(m).indexOf(",") + 2;
				}

				while( (arr2.size() > m) && (arr2.get(m-1).substring(indice1,arr2.get(m-1).length()).compareTo(arr2.get(m).substring(indice2,arr2.get(m).length())) <= 0 ) ){
				...
				for(int i = inicioAux + 1; i < arr.size(); i++){	
					StringBuilder temp = new StringBuilder(arr.get(i));	//de string a stringbuilder. Se usó...
								...
								//en esta seccion de una cadena auxiliar que cambia a lo largo de la ejecución
					h = i - 1;
					indice1 = arr.get(h).indexOf(",") + 2;
					indice2 = temp.toString().indexOf(",") + 2;
					while((h>=(inicioAux)) && (arr.get(h).substring(indice1,arr.get(h).length()).compareTo(temp.toString().substring(indice2,temp.toString().length())) >= 0)){	//stringbuilder a string
						arr.set(h+1,arr.get(h));
						h--;
	...
	public static void mezclaEquilibradaNumero(String nombreArchivo){    //recibira la direccion del archivo en cadena
		...
		do{
			...
			while( arr.size() != 0){	//mientras la lita original no este vacia.
				...
				if(arr.size() > 1){	//si hay mas de un elemento
					indice1 = arr.get(l-1).lastIndexOf(",") + 2;
					indice2 = arr.get(l).lastIndexOf(",") + 2;
					while( (arr.size() > l) && ((arr.get(l-1).substring(indice1,arr.get(l-1).length()).compareTo(arr.get(l).substring(indice2,arr.get(l).length()))) <= 0)){	//compara al elemento...
																						//...actual con el anterior...
																					//...siempre y cuando nos encontremos...
																				//...en indices validos.
			...
			while( (arr1.size() != 0) | (arr2.size() != 0) ){	//mientras una lista u otra no esté vacía
				...
				if(arr1.size() > 0){
					indice1 = arr1.get(k-1).lastIndexOf(",") + 2;
				}
				if(arr1.size() > 1){
					indice2 = arr1.get(k).lastIndexOf(",") + 2;
				}
				...
				if(arr2.size() > 0){
					indice1 = arr2.get(m-1).lastIndexOf(",") + 2;
				}
				if(arr2.size() > 1){
					indice2 = arr2.get(m).lastIndexOf(",") + 2;
				}
				...
				for(int i = inicioAux + 1; i < arr.size(); i++){	
					StringBuilder temp = new StringBuilder(arr.get(i));	//de string a stringbuilder. Se usó...
								...
								//en esta seccion de una cadena auxiliar que cambia a lo largo de la ejecución
					h = i - 1;
					indice1 = arr.get(h).lastIndexOf(",") + 2;
					indice2 = temp.toString().lastIndexOf(",") + 2;
					while((h>=(inicioAux)) && (arr.get(h).substring(indice1,arr.get(h).length()).compareTo(temp.toString().substring(indice2,temp.toString().length())) >= 0)){	//stringbuilder a string
						arr.set(h+1,arr.get(h));
						h--;
